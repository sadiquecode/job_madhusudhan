...
@foreach ($specialities as $speciality)
    <div class="headingtext text-center">
        <h6 class="fw-bold">({{ $speciality->title }})</h6>
    </div>
    <div class="d-flex flex-wrap justify-content-center">
        @foreach ($subjects->where('speciality_id', $speciality->id) as $subject)
...
<div class="col-md-12">
    ...
    <thead>
        <tr>
            ...
            <th scope="col-3">Specialized in</th>
            <th scope="col-3">Expertised in</th>
            <th scope="col-2">Phone number</th>
            <th scope="col-2">YEAR AND EXP</th>
            <th scope="col">Present working</th>
            ...
            <tr data-datatype="datatype-{{ $application->data_types_id }}" data-speciality="speciality-{{ $application->speciality_id }}" data-experty="experty-{{ $application->expertise_id }}" data-academic="academic-{{ $application->academic_id }}" data-non-academic="non_academic-{{ $application->non_academic_id }}" data-subject="subject-{{ $application->subject_id }}">
                ...
                <td>
                    @if (empty($application->profile_img))
                        <img class="rounded-5 profile-img" src="{{ url('public/assets/img/user.jpg') }}" alt="Profile Picture">
                    @else
