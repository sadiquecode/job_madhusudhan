...
<div class="col-sm-4 col-3">
    <h4 class="page-title">All Non Academic</h4>
</div>
<div class="col-sm-8 col-9 text-right m-b-20">
    <a href="#" class="btn btn-primary btn-rounded float-right" data-toggle="modal" data-target="#add_grade"><i class="fa fa-plus"></i> Add Non Academic</a>
</div>
</div>
@if ($errors->any())
...
<div class="col-md-12">
    <div class="table-responsive">
        <table class="table table-striped custom-table datatable"  id="teachme_table">
            ...
            <tbody>
                @foreach ($non_academic as $index => $grade)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ $grade->title }}</td>
                    <td>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content modal-md">
                            <div class="modal-header">
                                <h4 class="modal-title">Delete Non Academic</h4>
                            </div>
                            <form action="{{ route('non_academic.destroy', $grade->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
            ...
            <div id="edit_grade_{{ $grade->id }}" class="modal custom-modal fade" role="dialog">
                <div class="modal-dialog">
                    ...
                    <div class="modal-content modal-lg">
                        <div class="modal-header">
                            <h4 class="modal-title">Edit Non Academic</h4>
                        </div>
                        <div class="modal-body">
                            
                            <form class="m-b-30" action="{{ route('non_academic.update', $grade->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="row">
                                    <div class="col-sm-12">
                                ...
                                <div class="m-t-20 text-center mb-5">
                                    <button type="submit" class="btn btn-primary btn-lg">Update Non Academic</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
@else
<p class="p-4">{{'Non Academic Not Found!'}}</p>
@endif
</div>
</div>
</div>
...
<div class="modal-content modal-lg">
<div class="modal-header">
<h4 class="modal-title">Add Non Academic</h4>
</div>
<div class="modal-body">
<form class="m-b-30" action="{{ route('non_academic.store') }}" method="POST" enctype="multipart/form-data">
@csrf
<div class="row">
    <div class="col-sm-12">
        <div class="form-group">
...
<div class="m-t-20 text-center mb-5">
    <button type="submit" class="btn btn-primary btn-lg">Create Non Academic</button>
</div>
</form>
</div>
</div>
